 ...
 */
	protected function _getCSVLine(Model &$Model, SplFileObject $handle) {
        ...
        //kr problems with last line, returns empty array but it shouldn't?
        //so what, empty line -> finish
        if ($ret[0]==NULL)
            $ret=false;
        //kr bom-Stress
        //check BOM-Bytes 
        if ($ret)
        {
            ...
            if (strncmp($ret[0], $bom,3)==0)
            {
                $ret[0] = substr($ret[0], 3);
                $ret[0] = trim($ret[0],$this->settings[$Model->alias]['enclosure']);
            }    
        }
        return $ret;
	}
 ...
 */
	protected function _getHeader(Model &$Model, SplFileObject $handle) {
        ...
        else if (isset($this->settings[$Model->alias]['importKeys']))
        {
            /* extended by karo - if hasHeader is false and array importKeys is set, use them later*/
            $header=$this->settings[$Model->alias]['importKeys'];
        }
		else {
